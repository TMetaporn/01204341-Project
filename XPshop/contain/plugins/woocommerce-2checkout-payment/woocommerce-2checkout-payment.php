	...
	class WC_Gateway_NM_TwoCheckout extends WC_Payment_Gateway {
        ...
        */
		...
		function twocheckout_response() {
			...
			if( isset($_GET['wc-api']) && $_GET['wc-api'] == 'WC_Gateway_NM_TwoCheckout' && !isset($_REQUEST['fraud_status'] ) ) {
			...
			switch( $message_type ) {
				...
				case 'ORDER_CREATED':
					$wc_order->add_order_note( sprintf(__('ORDER_CREATED with Sale ID: %d', '2checkout'), $sale_id) );
					$this->log(sprintf(__('ORDER_CREATED with Sale ID: %d', '2checkout'), $sale_id));
				...
				case 'FRAUD_STATUS_CHANGED':
					if( $fraud_status == 'pass' ) {
						...
						$wc_order->add_order_note( sprintf(__('Payment Status Clear with Invoice ID: %d', '2checkout'), $invoice_id) );
						$this->log(sprintf(__('Payment Status Clear with Invoice ID: %d', '2checkout'), $invoice_id));
						add_action('twoco_order_completed', $order, $sale_id, $invoice_id);
						
					} elseif( $fraud_status == 'fail' ) {
						
						$wc_order->update_status('failed');
						$wc_order->add_order_note(  __("Payment Decliented", '2checkout') );
						$this->log( __("Payment Decliented", '2checkout') );
					}
